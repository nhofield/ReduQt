...
def main(n2f, number_of_iterations):
...
state_vector_constraints = []

while state_vector_constraint >= 1:
    ...
    state_vector_constraint = 0

    main(n2f, number_of_iterations)
    ...
    for state_index in range( len(outputstate) ):
        if outputstate[state_index] not in outputstate[n2f]:
            state_vector_constraint = abs( outputstate[state_index] )**2
            break

    state_vector_constraints.append(state_vector_constraint)
...
state_vector_constraints = np.array( state_vector_constraints )

print("Smallest State Vector Constraint")
print( np.min(state_vector_constraints) )
