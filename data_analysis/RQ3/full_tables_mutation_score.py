...
for program_category_index in range( len(program_categories) ):
    ...
    filename_latex_table = os.path.join( "tables", full_or_summary, f"latex_table_mutation_score_{curr_program_category}.txt" )
    ...
    with open( filename_latex_table , "w") as outfile:
        ...
        df_table = pd.DataFrame(columns = ["\\textbf{ID}", "Mutant Type", "\\textbf{Category}", "\\textbf{\\#Qubits}", "\\textbf{Depth}", "\\textbf{Default [\\%]}", "\\textbf{Greedy [\\%]}", "\\textbf{Random [\\%]}", "\\textbf{p-value} DFR", "\\textbf{p-value} DF", "$\\mathbf{\hat{A}}_{12}$ DF", "\\textbf{Magnitude} DF", "\\textbf{p-value} DR", "$\\mathbf{\hat{A}}_{12}$ DR", "\\textbf{Magnitude} DR", "\\textbf{p-value} FR", "$\\mathbf{\hat{A}}_{12}$ FR", "\\textbf{Magnitude} FR"] )
        ...
        for prog_index in range( len(unique_progs) ):
            ...
            df_default_current_program   = df_mutation_score_distribution_default[ df_mutation_score_distribution_default["Program var"] == current_program ]
            df_factoring_current_program = df_mutation_score_distribution_factoring[ df_mutation_score_distribution_factoring["Program var"] == current_program ]
            df_random_current_program    = df_mutation_score_distribution_random[ df_mutation_score_distribution_random["Program var"] == current_program ]

            # fetch number of qubits
            number_of_qubits = df_default_current_program["Qubits"][df_default_current_program.index[0]]

            # fetch depth
            depth = df_default_current_program[" depth"][df_default_current_program.index[0]]
